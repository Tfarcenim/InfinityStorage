...
public class InfinityBagContainer extends Container {

  protected InfinityBagHandler handler;
  ...
  public void addOwnSlots() {
    ...
    for (int row = 0; row < 3; ++row) {
      for (int col = 0; col < 9; ++col) {
        int x = 8 + col * 18;
        int y = row * 18 + 17 + 1;
        this.addSlotToContainer(new BagSlotExtended(handler, slotIndex, x, y));
        slotIndex++;
      }
    }
  }
  ...
  public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
    ...
    if (clickTypeIn == ClickType.QUICK_CRAFT) {
      ...
      if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
        ...
        if (slot7 != null && InfinityBagContainer.canAddItemToSlot(slot7, mouseStack, true) && slot7.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() > this.dragSlots.size()) && this.canDragIntoSlot(slot7)) {
        ...
        if (!this.dragSlots.isEmpty()) {
          ...
          for (Slot dragSlot : this.dragSlots) {
            ...
            if (dragSlot != null && InfinityBagContainer.canAddItemToSlot(dragSlot, mouseStack, true) && dragSlot.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() >= this.dragSlots.size()) && this.canDragIntoSlot(dragSlot)) {
      ...
      if (!mouseStack.isEmpty() && (slot == null || !slot.getHasStack() || !slot.canTakeStack(player))) {
        ...
        for (int k = 0; k < 2; ++k) {
          for (int l = i; l >= 0 && l < this.inventorySlots.size() && mouseStack.getCount() < mouseStack.getMaxStackSize(); l += j) {
            ...
            if (slot1.getHasStack() && InfinityBagContainer.canAddItemToSlot(slot1, mouseStack, true) && slot1.canTakeStack(player) && this.canMergeSlot(mouseStack, slot1)) {
