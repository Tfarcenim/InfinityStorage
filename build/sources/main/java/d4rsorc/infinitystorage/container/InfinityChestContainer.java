...
import d4rsorc.infinitystorage.network.MessageSyncExtendedSlotContents;
...
public class InfinityChestContainer extends Container {
  ...
  protected TileInfiniteStorage te;
  ...
  public InfinityChestContainer(TileInfiniteStorage te, EntityPlayer player) {
    this.te = te;
  ...
  public TileInfiniteStorage getTile() {
    return this.te;
  }
  ...
  public void addOwnSlots() {
    InfinityHandler handler = this.te.getHandler();
    int slotIndex = 0;
    for (int row = 0; row < 3; ++row) {
      for (int col = 0; col < 9; ++col) {
        int x = 8 + col * 18;
  ...
  public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
    ...
    if (clickTypeIn == ClickType.QUICK_CRAFT) {
      ...
      if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
        ...
        if (slot7 != null && InfinityChestContainer.canAddItemToSlot(slot7, mouseStack, true) && slot7.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() > this.dragSlots.size()) && this.canDragIntoSlot(slot7)) {
        ...
        if (!this.dragSlots.isEmpty()) {
          ...
          for (Slot dragSlot : this.dragSlots) {
            ...
            if (dragSlot != null && InfinityChestContainer.canAddItemToSlot(dragSlot, mouseStack, true) && dragSlot.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() >= this.dragSlots.size()) && this.canDragIntoSlot(dragSlot)) {
      ...
      if (!mouseStack.isEmpty() && (slot == null || !slot.getHasStack() || !slot.canTakeStack(player))) {
        ...
        for (int k = 0; k < 2; ++k) {
          for (int l = i; l >= 0 && l < this.inventorySlots.size() && mouseStack.getCount() < mouseStack.getMaxStackSize(); l += j) {
            ...
            if (slot1.getHasStack() && InfinityChestContainer.canAddItemToSlot(slot1, mouseStack, true) && slot1.canTakeStack(player) && this.canMergeSlot(mouseStack, slot1)) {
