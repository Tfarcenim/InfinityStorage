...
import d4rsorc.infinitystorage.RenderItemExtended;
...
public class InfinityBagScreen extends GuiContainer {

  public static final int WIDTH = 176;
  public static final int HEIGHT = 166;

  private static final ResourceLocation background = new ResourceLocation("textures/gui/container/shulker_box.png");
  ...
  private InfinityBagHandler te;
  private InventoryPlayer playerinventory;

  public InfinityBagScreen(InfinityBagContainer container, InventoryPlayer playerinventory) {
    super(container);

    this.playerinventory = playerinventory;
    this.te = container.handler;
    xSize = WIDTH;
    ySize = HEIGHT;
  ...
  private void drawSlot(Slot slotIn) {
    ...
    if (slotIn == this.clickedSlot && !this.draggedStack.isEmpty() && this.isRightMouseClick && !itemstack.isEmpty()) {
      ...
      if (InfinityChestContainer.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
    ...
    if (!flag1) {
      ...
      RenderItemExtended.INSTANCE.setZLevel(this.itemRender.zLevel);
      if (slotIn instanceof BagSlotExtended) {
  ...
  protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
    ...
    if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
      if (clickedMouseButton == 0 || clickedMouseButton == 1) {
        ...
        } else if (this.draggedStack.getCount() > 1 && slot != null && InfinityChestContainer.canAddItemToSlot(slot, this.draggedStack, false)) {
          long i = Minecraft.getSystemTime();
    ...
    } else if (this.dragSplitting && slot != null && !itemstack.isEmpty() && (itemstack.getCount() > this.dragSplittingSlots.size() || this.dragSplittingLimit == 2) && InfinityChestContainer.canAddItemToSlot(slot, itemstack, true) && slot.isItemValid(itemstack) && this.inventorySlots.canDragIntoSlot(slot)) {
    ...
    if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(ItemStack.EMPTY, slot)) {
      if (isShiftKeyDown()) {
        if (!this.shiftClickedSlot.isEmpty()) {
          for (Slot slot2 : this.inventorySlots.inventorySlots) {
            if (slot2 != null && slot2.canTakeStack(this.mc.player) && slot2.getHasStack() && slot2.isSameInventory(slot) && InfinityChestContainer.canAddItemToSlot(slot2, this.shiftClickedSlot, true)) {
      ...
      if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
        if (state == 0 || state == 1) {
          ...
          boolean flag2 = InfinityChestContainer.canAddItemToSlot(slot, this.draggedStack, false);
